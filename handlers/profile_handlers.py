...
async def get_profile(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("Выберете профиль кандидата, если нужного профиля нет, введите вручную",
...
async def get_stack(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Укажите стек кандидата", reply_markup=cancel_markup)
...
async def get_stack(mess: Message, state: FSMContext):
    ...
    await mess.answer("Укажите стек кандидата", reply_markup=cancel_markup)
    await state.set_state(Fsm_anket.experience)
...
async def get_coment(mess: Message, state: FSMContext):
    ...
    await mess.answer("Ваши комментарии (если навигируешь кандидата ОБЯЗАТЕЛЬНО укажи комментарий, в остальных случаях - опционально, если вдруг комментариев нет напиши нет)", reply_markup=cancel_markup)
...
async def get_resume(mess: Message, state: FSMContext):
    ...
    await mess.answer("Укажите ссылку на резюме в Пульс", reply_markup=cancel_markup)
...
async def get_hastag(mess: Message, state: FSMContext):
    ...
    await mess.answer("Укажите Хештеги (рекрутер указывает 3 хэштега а сорсер - 4.  ", reply_markup=cancel_markup)
